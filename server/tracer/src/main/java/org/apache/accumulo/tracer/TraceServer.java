...
public class TraceServer implements Watcher {
    ...
    public TraceServer(Instance instance, ServerConfigurationFactory serverConfiguration, String hostname) throws Exception {
        ...
        writer = new AtomicReference<>(this.connector.createBatchWriter(tableName, new BatchWriterConfig().setMaxLatency(BATCH_WRITER_MAX_LATENCY, TimeUnit.SECONDS)));
    }
   ...
   */
    private Connector ensureTraceTableExists(final AccumuloConfiguration conf) throws AccumuloSecurityException, ClassNotFoundException, InstantiationException, IllegalAccessException {
        ...
        while (true) {
            try {
                ...
                break;
            } catch (AccumuloExceptionTableExistsException | TableNotFoundException | IOException | RuntimeException |  ex) {
    ...
    private static void loginTracer(AccumuloConfiguration acuConf) {
        try {
            Class<? extends AuthenticationToken> traceTokenType = AccumuloVFSClassLoader.getClassLoader().loadClass(acuConf.get(Property.TRACE_TOKEN_TYPE)).asSubclass(AuthenticationToken.class);
        ...
        } catch (IOExceptionClassNotFoundException |  exception) {
